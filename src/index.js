...
/**
 * Source Connector for Agora
 * 
 * This connector allows extracting data from Agora's real-time communication platform
...
const CallQualityStream = require('./streams/call_quality');
const RecordingsStream = require('./streams/recordings');
const ChannelsStream = require('./streams/channels');
const EventsStream = require('./streams/events');

// Available streams in this connector
const AVAILABLE_STREAMS = {
  usage: UsageStream,
  call_quality: CallQualityStream,
  recordings: RecordingsStream,
  channels: ChannelsStream,
  events: EventsStream
};
...
async function check(configJson) {
  try {
    ...
    const usageStream = new UsageStream(mergedConfig);
    await usageStream.testConnection();
...
program
  .name('source-agora')
  .description('Agora source connector')
  .version('0.1.0');
