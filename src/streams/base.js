/**
 * Base stream implementation for all Agora streams
 */
const fs = require('fs');
...
class BaseStream {
  ...
  /**
   * Create an authenticated Agora API client
   * @returns {Object} The API client
   */
  createClient() {
    const { app_id, customer_id, customer_secret, endpoint_url, region } = this.config;
    
    // Set up base URL based on region if provided
    let baseURL = endpoint_url;
    if (region && region !== 'global') {
      baseURL = baseURL.replace('api.agora.io', `api-${region}.agora.io`);
    }
    ...
    // Create signature based on Agora's authentication requirements
    const signatureString = `${customer_id}${timestamp}${nonce}`;
    ...
    const client = axios.create({
      baseURL,
      headers: {
        'x-agora-token': signature,
        'x-agora-appid': app_id,
        'x-agora-customerid': customer_id,
        'x-agora-timestamp': timestamp.toString(),
        'x-agora-nonce': nonce,
  ...
  /**
   * Test the connection to the Agora API
  ...
  async testConnection() {
    ...
    try {
      // Make a simple API request to verify connectivity (modify endpoint as needed)
      await client.get('/v1/apps/info');
      return true;
    } catch (error) {
